...
#include "CrystalPlasticityKalidindiSlipTwinUpdate.h"
#include "libmesh/int_range.h"

registerMooseObject("SolidMechanicsApp", CrystalPlasticityKalidindiSlipTwinUpdate);

InputParameters
CrystalPlasticityKalidindiSlipTwinUpdate::validParams()
{
  ...
  params.addParam<Real>("h", 900, "hardening constants");
  params.addParam<Real>("tau1", 790, "initial saturated slip system strength");
  params.addParam<Real>("theta1", 280, "initial saturated slip system strenthening coefficent");
  params.addParam<Real>("gss_a", 1.0, "coefficient for hardening");
  params.addParam<Real>("ao", 3E4, "slip rate coefficient");
  params.addParam<Real>("xm", 0.05, "exponent for slip rate");
  params.addParam<Real>("gss_initial", 345, "initial lattice friction strength of the material");
...
CrystalPlasticityKalidindiSlipTwinUpdate::CrystalPlasticityKalidindiSlipTwinUpdate(
    const InputParameters & parameters)
  : CrystalPlasticityStressUpdateBase(parameters),
    // Constitutive values
    _r(getParam<Real>("r")),
    _h(getParam<Real>("h")),
    _tau1(getParam<Real>("tau1")),
...
void
CrystalPlasticityKalidindiSlipTwinUpdate::initQpStatefulProperties()
{
...
void
CrystalPlasticityKalidindiSlipTwinUpdate::setInitialConstitutiveVariableValues()
{
...
void
CrystalPlasticityKalidindiSlipTwinUpdate::setSubstepConstitutiveVariableValues()
{
...
bool
CrystalPlasticityKalidindiSlipTwinUpdate::calculateSlipRate()
{
  for (const auto i : make_range(_number_slip_systems))
  {
...
void
CrystalPlasticityKalidindiSlipTwinUpdate::calculateEquivalentSlipIncrement(
...
void
CrystalPlasticityKalidindiSlipTwinUpdate::calculateConstitutiveSlipDerivative(
...
bool
CrystalPlasticityKalidindiSlipTwinUpdate::areConstitutiveStateVariablesConverged()
{
...
void
CrystalPlasticityKalidindiSlipTwinUpdate::updateSubstepConstitutiveVariableValues()
{
  // Would also set substepped dislocation densities here if included in this model
...
void
CrystalPlasticityKalidindiSlipTwinUpdate::cacheStateVariablesBeforeUpdate()
{
  _slip_resistance_before_update = _slip_resistance[_qp];
}

void
CrystalPlasticityKalidindiSlipTwinUpdate::calculateStateVariableEvolutionRateComponent()
{
  Real tau_sat = _tau1 + _theta1 * std::pow((*_twin_volume_fraction_total)[_qp], 0.5);
  for (const auto i : make_range(_number_slip_systems))
  {
    // Clear out increment from the previous iteration
    _slip_resistance_increment[i] = 0.0;

    _hb[i] = _h * std::pow(std::abs(1.0 - _slip_resistance[_qp][i] / tau_sat), _gss_a);
    const Real hsign = 1.0 - _slip_resistance[_qp][i] / tau_sat;
    if (hsign < 0.0)
      _hb[i] *= -1.0;
  }
...
bool
CrystalPlasticityKalidindiSlipTwinUpdate::updateStateVariables()
{
